...
@section('content')
    <div class="question-form-page">
        <section class="question-form-hero">
            <p class="question-form-kicker">Soal</p>
            <h1 class="question-form-title">Tambah soal untuk misi</h1>
        </section>
        ...
        <form id="question-form" action="{{ route('lecturer.questions.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="question-form-grid two">
                <div>
                    @php
                        $selectedmisiId = old('challenge_id', $selectedChallengeId ?? '');
                        $selectedmisi = $challenges->firstWhere('id', (int) $selectedmisiId);
                    @endphp
                    <label for="misi-search" class="question-form-label">Misi</label>
                    <div class="question-misi-picker">
                        <input type="hidden" name="challenge_id" id="challenge_id" value="{{ $selectedmisiId }}" required>
                        <button type="button" id="misi-picker-button" class="question-form-input question-misi-button"
                            onclick="togglemisiPicker()">
                            <span id="misi-picker-label">
                                {{ $selectedmisi ? (($selectedmisi->section?->order ?? '-') . '. ' . $selectedmisi->title) : 'Pilih misi' }}
                            </span>
                            <span>v</span>
                        </button>
                        <div id="misi-picker-menu" class="question-misi-menu hidden">
                            <input type="text" id="misi-search" class="question-misi-search"
                                placeholder="Cari misi atau bagian materi..." oninput="filtermisiOptions()">
                            <div class="question-misi-options">
                                @foreach ($challenges as $challenge)
                                    @php
                                        $misiLabel = ($challenge->section?->order ?? '-') . '. ' . $challenge->title;
                                    @endphp
                                    <button type="button" class="question-misi-option"
                                        data-value="{{ $challenge->id }}"
                                        data-label="{{ $misiLabel }}"
                                        onclick="selectmisi(this)">
                                        {{ $misiLabel }}
                                    </button>
                                @endforeach
                            </div>
            ...
            <div class="question-block-panel">
                ...
                <p class="question-block-copy">Atur teks dan gambar sesuai urutan yang akan dilihat mahasiswa.</p>
                <div id="blocks-body" class="question-block-list"></div>
            </div>
            ...
            <div class="question-form-grid one">
                ...
                <div>
                    ...
                    <input type="file" name="question_image" id="question_image" class="question-form-input file-input" accept="image/*">
            ...
            <div class="question-block-panel explanation-panel">
                ...
                <p class="question-block-copy">Atur teks dan gambar sesuai urutan pembahasan. Cocok untuk format: teks, gambar, teks lagi, lalu gambar solusi.</p>
                <div id="explanation-blocks-body" class="question-block-list"></div>
            </div>
        ...
        <div id="saving-overlay" class="question-saving-overlay hidden">
            <div class="question-saving-card">
                ...
                <p class="question-saving-title">Menyimpan soal...</p>
                <p class="question-saving-copy">Tunggu sebentar, isi soal dan gambar sedang diproses.</p>
            </div>
        </div>
    </div>
    ...
    <style>
        ...
        .question-misi-picker { position: relative; }
        .question-misi-button { display: flex; align-items: center; justify-content: space-between; gap: 12px; text-align: left; }
        .question-misi-menu { position: absolute; z-index: 50; top: calc(100% + 8px); left: 0; right: 0; padding: 12px; border-radius: 20px; border: 1px solid #9CB8D8; background: #fff; box-shadow: 0 24px 60px rgba(15, 23, 42, 0.18); }
        .question-misi-menu.hidden { display: none; }
        .question-misi-search { width: 100%; margin-bottom: 10px; padding: 12px 14px; border-radius: 14px; border: 1px solid #e5e7eb; color: #09254A; background: #fff; box-sizing: border-box; }
        .question-misi-options { max-height: 280px; overflow-y: auto; display: grid; gap: 6px; }
        .question-misi-option { width: 100%; padding: 12px 14px; border: 0; border-radius: 12px; background: #E8F0F8; color: #09254A; text-align: left; cursor: pointer; font-weight: 600; }
        .question-misi-option:hover { background: #DCE7F3; color: #1D5FD6; }
        ...
        .answer-row input[type="text"], .answer-row input[type="file"], .answer-row textarea { width: 100%; padding: 12px 14px; border-radius: 14px; border: 1px solid #B7CCE6; box-sizing: border-box; color: #09254A !important; background: #fff !important; }
    ...
    <script>
        ...
        function togglemisiPicker() {
            const menu = document.getElementById('misi-picker-menu');
            menu.classList.toggle('hidden');

            if (!menu.classList.contains('hidden')) {
                document.getElementById('misi-search').focus();
            }
        }

        function selectmisi(option) {
            document.getElementById('challenge_id').value = option.dataset.value;
            document.getElementById('misi-picker-label').textContent = option.dataset.label;
            document.getElementById('misi-picker-menu').classList.add('hidden');
            document.getElementById('misi-search').value = '';
            filtermisiOptions();
        }

        function filtermisiOptions() {
            const search = document.getElementById('misi-search').value.toLowerCase();
            document.querySelectorAll('.question-misi-option').forEach((option) => {
                option.style.display = option.dataset.label.toLowerCase().includes(search) ? '' : 'none';
            });
        }

        document.addEventListener('click', function(event) {
            const picker = document.querySelector('.question-misi-picker');
        ...
        function resetToEssay() {
            answersBody.innerHTML = `<tr class="answer-row"><td colspan="2"><textarea name="answers[]" rows="3" required placeholder="Tulis kunci jawaban esai yang dianggap benar."></textarea></td><td class="text-center text-slate-500">Kunci esai</td><td class="text-center text-slate-500">-</td></tr>`;
        ...
        function resetToMultipleChoice() {
            answersBody.innerHTML = `<tr class="answer-row"><td><input type="text" name="answers[]" required></td><td><input type="file" name="answer_images[]" accept="image/*"></td><td class="text-center"><input type="hidden" name="is_correct[]" value="0" class="is-correct-hidden"><label class="correct-checkbox-wrap"><input type="checkbox" class="correct-checkbox" onchange="toggleCorrectAnswer(this)"><span>Pilih</span></label></td><td class="text-center"><button type="button" class="question-form-btn" style="background:#ef4444;padding:10px 14px;" onclick="removeAnswer(this)">Hapus</button></td></tr>`;
            addAnswerBtn.style.display = 'inline-flex';
        }

        function addAnswer() {
            const newRow = `<tr class="answer-row"><td><input type="text" name="answers[]" required></td><td><input type="file" name="answer_images[]" accept="image/*"></td><td class="text-center"><input type="hidden" name="is_correct[]" value="0" class="is-correct-hidden"><label class="correct-checkbox-wrap"><input type="checkbox" class="correct-checkbox" onchange="toggleCorrectAnswer(this)"><span>Pilih</span></label></td><td class="text-center"><button type="button" class="question-form-btn" style="background:#ef4444;padding:10px 14px;" onclick="removeAnswer(this)">Hapus</button></td></tr>`;
        ...
        function showSubmitError(messages) {
            ...
            alertBox.innerHTML = `<strong>Data belum bisa disimpan.</strong><ul>${messages.map((message) => `<li>${message}</li>`).join('')}</ul>`;
        ...
        document.getElementById('question-form').addEventListener('submit', async function (event) {
            ...
            try {
                ...
                showSubmitError(['Server belum memberi balasan dalam 90 detik. Soal mungkin sudah tersimpan; cek daftar soal dulu sebelum mencoba lagi.']);
            } finally {
